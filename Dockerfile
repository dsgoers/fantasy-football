...
USER root
RUN apt-get -y update; apt-get -y install cron; apt-get -y install curl; apt-get install -y openjdk-17-jre; curl -LO https://github.com/tidbyt/pixlet/releases/download/v0.29.1/pixlet_0.29.1_linux_amd64.tar.gz; tar -xvf pixlet_0.29.1_linux_amd64.tar.gz; chmod +x ./pixlet; mv pixlet /usr/local/bin/pixlet; echo "* * * * * /fantasy_football/tidbyt.sh" | crontab -; service cron start;
